...
A jornada única **Acessar minha loja** está implementada localmente em `/painel/acessar-loja`: resposta não enumerável, rate limit por IP e HMAC do e-mail, token aleatório de uso único com 20 minutos, somente hash no banco e envio pela API do Resend. Depois da comprovação do e-mail, um roteador servidor identifica conta pronta, primeira senha, checkout válido, pagamento em confirmação, checkout reiniciável, cancelamento retido ou expurgo. Checkout válido é reutilizado e o reinício usa claim atômico; pagamento continua confirmado exclusivamente pelo webhook.
...
| `/painel/acessar-loja` | Implementada localmente | Jornada pública única e neutra para acessar uma contratação em qualquer estado, sem enumerar contas. |
| `/painel/acessar-loja/confirmar` | Implementada localmente | Consome explicitamente um token de uso único mantido no fragmento da URL e restaura o cookie HTTP-only de retomada. |
| `/painel/acessar-loja/continuar` | Implementada localmente | Consulta o estado real no servidor e oferece somente a ação segura aplicável. |
| `/cadastro/recuperar` | Compatibilidade | Redireciona para a jornada única em `/painel/acessar-loja`. |
| `/cadastro/recuperar/confirmar` | Compatibilidade | Links antigos continuam consumindo o mesmo token e seguem para a jornada nova. |
...
| `POST /api/cadastro/continuar-checkout` | Implementado localmente | Revalida o estado e reserva atomicamente a recriação do checkout contra clique duplo. |
| `POST /api/cadastro/checkout-retornado` | Implementado localmente | Registra apenas evidência visual de retorno; nunca confirma pagamento. |
...
| 5 | Timeout/aparelho perdido após pagamento | **Coberto localmente.** Além do polling com timeout, o titular usa “Acessar minha loja” em qualquer dispositivo, comprova o e-mail e recebe a ação correta. A conta pronta usa login/reset oficial; primeira senha exige intenção paga e owner correspondente; checkout válido é reutilizado e um expirado só é recriado após clique e claim atômico. | Aplicar a migration 016 e executar os cenários cross-device no deploy. |
