...
export function AccountContinuation({ reference }: { reference: string | null }) {
  ...
  if (!reference) {
    return <MessageCard title="Solicite um novo link" text="Por segurança, use novamente o e-mail informado no ClickCatálogo."><Link className={buttonVariants()} href="/painel/acessar-loja">Enviar novo link</Link></MessageCard>;
  }
  if (loading) {
  ...
  if (error && !state) {
    return <MessageCard title="Não foi possível consultar agora" text={error}><Button onClick={() => void refresh()}><RotateCw aria-hidden="true" />Tentar novamente</Button><Link className={buttonVariants({ variant: "secondary" })} href="/painel/acessar-loja">Solicitar novo link</Link></MessageCard>;
  }
  ...
  if (state.type === "ACCOUNT_READY") {
    return <MessageCard icon="success" title="Sua conta já está pronta" text="Este e-mail já possui acesso ao ClickCatálogo."><Link className={buttonVariants()} href="/painel">Entrar na minha conta</Link><Link className={buttonVariants({ variant: "secondary" })} href="/painel/recuperar-senha">Redefinir senha</Link></MessageCard>;
  }
  ...
  if (state.type === "REACTIVATION_PENDING") {
    return <MessageCard icon="waiting" title="Estamos conferindo sua assinatura" text="Encontramos sua loja e existe uma atualização financeira em andamento. Entre na conta para acompanhar; nenhuma nova cobrança será criada por esta tela."><Link className={buttonVariants()} href="/painel">Entrar na minha conta</Link><Link className={buttonVariants({ variant: "secondary" })} href="/painel/recuperar-senha">Redefinir senha</Link></MessageCard>;
  }
  if (state.type === "CANCELED_RETAINED") {
    return <MessageCard title="Sua loja pode ser reativada" text="Encontramos sua loja anterior. Entre na conta para reativá-la sem cadastrar seus produtos novamente."><Link className={buttonVariants()} href="/painel">Entrar para reativar</Link><Link className={buttonVariants({ variant: "secondary" })} href="/painel/recuperar-senha">Redefinir senha</Link></MessageCard>;
  }
  return <MessageCard title="Não encontramos uma loja disponível para recuperação" text="Você pode começar uma nova loja com este e-mail."><Link className={buttonVariants()} href="/cadastro">Criar uma nova loja</Link><Link className={buttonVariants({ variant: "secondary" })} href="/painel">Voltar para o login</Link></MessageCard>;
}
